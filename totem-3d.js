...
function buildPrincipal() {
  ...
  const c5backZ = c5frontZ - c5depth;         // borda de fundo do C5 (lado da tela)
  const telaZ = c5backZ;                       // tela no fundo do C5
  const furoTela = 30;                         // furo a 3 cm da tela
  const roscaZ = telaZ + furoTela;             // centro da rosca 1/4" no C5
  ...
  const camBody = piece(
    ...
    { cod: 'CAM', nome: 'Corpo Canon EOS Rebel T7', mat: 'DSLR APS-C', l: 140, a: 100, obs: 'Tela no fundo do C5; rosca 1/4" a 30 mm (3 cm) da tela' }
  );
  ...
  const rosca = piece(
    ...
    { cod: 'C5', nome: 'Rosca 1/4" (soquete tripé)', obs: 'Furo a 30 mm (3 cm) da tela, no fundo do C5' }
  );
